...
DvtStatusMeterGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  if(isRTL)
    plotAreaEnd = Math.min(bounds.x, Math.max(bounds.x + bounds.w, plotAreaEnd));
  
  else
    plotAreaEnd = Math.max(bounds.x, Math.max(bounds.x + bounds.w, plotAreaEnd));
  ...
  if (thresholds && options['plotArea']['rendered'] != 'off' && options['thresholdDisplay'] == 'all') {
    for (var i = thresholds.length - 1; i >= 0; i--) {
      ...
      else
      {
        ...
        cp.addRect(0, 0, (Math.abs(thresholds[currentThresholdIndex]['max'] - options['min'])) * 1 / (Math.abs(options['min'] - options['max'])) * bounds.w, plotY2 - plotY1 + 2, 0, 0);
      }
