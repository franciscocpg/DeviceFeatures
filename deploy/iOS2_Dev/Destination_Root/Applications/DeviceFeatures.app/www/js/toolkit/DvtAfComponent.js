 ...
 */
var DvtText = function(context, textString, x, y, id)
{
  this.Init(context, textString, x, y, id);
};
 ...
 * @override
 */
DvtText.prototype.Init = function(context, textString, x, y, id)
{
  DvtText.superclass.Init.call(this, context, 'text', id);
  ...
  if (DvtAgent.isRightToLeft(context)) {
    if (DvtAgent.isPlatformIE())
      this.setTextString('\u202B' + textString);
    else if (DvtText.needsTextAnchorAdjustment(context))
      this.alignLeft();
    else
...
DvtText.prototype.impl_alignLeft = function() {
  ...
  var bAdjust = DvtText.needsTextAnchorAdjustment(this._context);
...
DvtText.prototype.impl_setCSSStyle = function(style)
{
  ...
  if (style) {
    ...
    if (val) {
      ...
      if (val !== 'underline' ||
          !(DvtAgent.isPlatformGecko() && DvtAgent.getVersion() >= 25 && this._context.getRootAttribute('text-rendering') === 'geometricPrecision'))
...
DvtText.prototype.impl__handleHtmlTextAreaKeyboardEvent = function(event) {
  ...
  this.getObj().dispatchDisplayableEvent(event.type, '' + event.keyCode);
};
...
DvtText.prototype.impl__createHtmlTextArea = function(obj, textElem, editText) {
   ...
   css += ' color:' + DvtToolkitUtils.getAttrNullNS(textElem, 'fill') + ';';
   css += ' overflow-x: hidden;';
   css += ' border: none; border-width: 0px; margin: 0px; resize: none; outline: none;';
   css += ' dominant-baseline:' + DvtToolkitUtils.getAttrNullNS(textElem, 'dominant-baseline') + ';';
...
// css += " line-height: 1;" ;
   ...
   css += ' position: absolute; top: ' + stagePoint.y + 'px; left: ' + stagePoint.x + 'px;';

   css += ' vertical-align: baseline; outline-offset: 0px;';
   var padding = 2;
   if (DvtAgent.isRightToLeft(this.getObj().getCtx())) {
     css += ' text-align: right; padding: 0px ' + padding + 'px 0px 0px;';
   }
   else {
     css += ' text-align: left; padding: 0px 0px 0px ' + padding + 'px;';
   }

   input.setAttribute('style', css);

   if (DvtAgent.isBrowserChrome() || DvtAgent.isBrowserSafari()) {
 ...
 * @return {boolean}
 */
DvtText.needsTextAnchorAdjustment = function(context) {
  // Bug 17776065: When html dir="rtl", Webkit and FF25+ treat the right side of the text as the start, and the left
  // side of the text as end.  Our API always treats the left side as start, so we need to adjust based on agent.
  ...
  return DvtAgent.isRightToLeft(context) &&
...
DvtTextArea.prototype.Init = function(context, x, y, id) {
  ...
  DvtTextArea.superclass.Init.call(this, context, '', x, y, id);
...
DvtTextFormatted.prototype.Init = function(context, x, y, id) {
  DvtTextFormatted.superclass.Init.call(this, context, '', x, y, id);
 ...
 * @constructor
 */
var DvtAfContext = function(context, eventManager) {
  this.Init(context, eventManager);
};

DvtObj.createSubclass(DvtAfContext, DvtObj, 'DvtAfContext');

DvtAfContext.prototype.Init = function(context, eventManager) {
  this._context = context;
  this._dimInShape = false;          // whether dimensions already cached in shapes
  this._clearCachedObjList();
//  this._dirty = 0;
  this.setEventManager(eventManager);

  //reset the id generate so we wont create the same clipPath again
...
/**
 * get a list of (Id, Shape) pairs
 * those are objects need to getDimensions later
 */
DvtAfContext.prototype.GetObjShapeList = function() {
  if (! this._cachedObjList)
    this._cachedObjList = {};

  return this._cachedObjList;
};


/**
 * get the object count of the list of (Id, Shape) pairs
 */
// DvtAfContext.prototype.GetObjDimCnt = function() {
//   return this._cachedObjCnt;
// };

/**
 * add an entry to the list of (Id, Shape) pairs
 */
DvtAfContext.prototype.AddObjShapePair = function(id, shape) {
  var list = this.GetObjShapeList();
...
DvtAfContext.prototype._getObjDimList = function() {
  if (! this._dimInShape && this._cachedObjCnt) {
    var list = this.GetObjShapeList();
    var shape;
    for (var id in list) {
      shape = list[id];
      // getDimensions and cached in shape
      shape.getCachedDims();
    }
    this._dimInShape = true;
  }
  return this._cachedObjList;
};
...
/**
 * getDimensions for all shapes in objDimList
 */
DvtAfContext.prototype.GetDimensions = function() {
  this._getObjDimList();

  //Note: destroy the getDimensions object list
  this._clearCachedObjList();
};
...
/**
 * Truncate texts for all shapes in objDimList
 */
DvtAfContext.prototype.TruncateTexts = function() {
  if (this._cachedObjList) {

    var list = this.GetObjShapeList();
    var textFields = [];
    for (var id in list) {
      textFields.push(list[id]);
    }
    DvtText.truncateTexts(textFields);

    //Note: destroy the pending truncation object list
    this._clearCachedObjList();
  }
};
 ...
 */
DvtAfContext.prototype._clearCachedObjList = function() {
  this._cachedObjCnt = 0;
  this._cachedObjList = undefined;
};
...
/**
 * Add dirty when an image is loaded
 * @param {number} amt 1 for increment and -1 for decrement

DvtAfContext.prototype._addDirty = function(amt) {
  this._dirty = Math.max(this._dirty + amt, 0);

  //fire refresh event
  if (amt === -1 && this._refreshCallback) {
    this._refreshCallback(this._dirty);
  }
};

DvtAfContext.prototype._isDirty = function() {
 ...
 * @override
 */
DvtAfComponent.prototype.setProperty = function(propName, val) {
  DvtAfComponent.superclass.setProperty.call(this, propName, val);
};
...
DvtAfComponent.prototype.render = function(context) {

  this._context = context;
  this._render(DvtAfComponent.ParseNonNegativeFloat(context.getAvailableWidth()),
               DvtAfComponent.ParseNonNegativeFloat(context.getAvailableHeight()));

  //TODO: clear this._context member variable after render, because some of its info goes stale
};
...
//     this.createComputedStyle();
  ...
  if (shape && this.isTooltipEnabled() && (! this._context.getRmIfNotFit())) {
  ...
  if (this._context.getRmIfNotFit()) {
...
DvtAfComponent.prototype.createComputedStyle = function() {
  if (! this._computedStyle) {
    ...
    if (DvtAfComponentFactory.CACHE_STYLES) {
      ...
      cssStyle = this._context.GetCSSStyle(this.getTagName(), sc, ins, isText);
    }
    else {
...
DvtAfComponent.prototype.resolveEmUnit = function(cssStyle, startComp) {
  ...
  if (cssStyle) {
    ...
    if (! fontSize) {
      fontSize = this._context.getFontSize();
    }
...
//   else if (DvtAgent.isRightToLeft(this._context.getCtx())) {
...
DvtAfComponent.prototype.getEffectiveHAlign = function() {
  ...
  if (hAlign == DvtAfComponent.HALIGN_START) {
    if (DvtAgent.isRightToLeft(this._context.getCtx()))
...
/**
 * @param {DvtAfContext} context The rendering context which should contain width, height and optional fontSize and childTagHandler.
 * @return {DvtAfComponent} the root of the DvtAfComponent tree
 */
DvtAfComponent.prototype.createDisplayObj = function(context) {
  ...
  this._parentNode = context.getParentNode();

  //TODO: clear this._context member variable after render, because some of its info goes stale
  this._context = context;
  var icontext = context.getCtx();
  ...
  // create shape
  this._shape = this.createContentShape(icontext);

  // create background shape
  this._bgShape = this.createBackgroundShape(icontext, this._shape);
...
DvtAfComponent.prototype.addChildDisplayObjs = function(context, parentDO) {
  var child;
  var childDO;
  var cnt = this.getNumChildren();
  ...
  for (var i = 0; i < cnt; i++) {
    child = this.getChildAt(i);
    if (! child.renderMe || child.renderMe()) {
      childDO = child.createDisplayObj(context);
      if (childDO) {
...
DvtAfComponent.prototype.createContentShape = function(icontext) {
  return null;
};
...
DvtAfComponent.prototype.createBackgroundShape = function(icontext, shape) {
  var bgShape;

  if (this._hasBgShape()) {
    bgShape = new DvtRect(icontext, 0, 0, 0, 0, this._getUniqueId() + '_bg');
...
DvtAfComponent.prototype.RenderBackgroundFill = function(bgShape, style, bgColor, bgImg) {
  if (bgImg && (bgImg instanceof DvtCSSGradient)) {
    ...
    //if background color is not specified, ignore
    if (! bgColor)
      return;
...
DvtAfComponent.prototype.RenderBackgroundStroke = function(bgShape, style) {
  ...
  if (bdColor) {
    ...
    bgShape.setSolidStroke(bdColor, null, bdWidth);
  }
...
DvtAfComponent.prototype.RenderBackgroundImage = function(bgShape, style, bgImg) {
  if (bgImg && 'none' !== bgImg && (! (bgImg instanceof DvtCSSGradient))) {
    ...
    this._bgImg = bgImg;
    DvtImageLoader.loadImage(this._context.getCtx(),
                             bgImg,
...
DvtAfComponent.prototype.createTextShape = function(isWordWrap, text, 
  ...
  if (isWordWrap) {
    //TODO remove if graph is ok with the text alignment changes
    // textShape = new DvtTextArea(icontext, 0, 0, id);
    textShape = new DvtTextArea(icontext, undefined, undefined, id);
  }
  else {
    textShape = new DvtText(icontext, null, 0, 0, id);
  }

  // defer text truncation
  var deferTruncation = this.deferTruncation ? this.deferTruncation() : false;
  if (deferTruncation)
    textShape.setTruncationDeferred(deferTruncation);
  ...
  if (isWordWrap) {
    textShape.setWordWrap(isWordWrap);
  }

  if (this.getTruncateAt) {
    textShape.setTruncateAt(this.getTruncateAt());
  }

  // add to a list of obj that need to getDimensions
  if (deferTruncation && (! isWordWrap)) {
    context.AddObjShapePair(id, textShape);
  }
...
DvtAfComponent.prototype.SaveContextCallback = function() {
  if ((this.getShowPopupBehaviors && this.getShowPopupBehaviors()) || this.getClientBehaviors() ||
    ...
    this._contextCallbackObj = this._context.getContextCallbackObj();
    this._contextCallback = this._context.getContextCallback();
  }
...
DvtAfComponent._renderComp = function(context, comp, availWidth, availHeight) {

  //save available width and height
  var saveAW = context.getAvailableWidth();
  var saveAH = context.getAvailableHeight();
  context.setAvailableWidth(availWidth);
  context.setAvailableHeight(availHeight);

  if (comp.render) {
    comp.render(context);
  }
  //restore available width and height
  context.setAvailableWidth(saveAW);
  context.setAvailableHeight(saveAH);

};


// This method is called when this component is too big for its container
DvtAfComponent.prototype._addClipPathIfNeeded = function(context, xx, yy, aw, dimW, ah, dimH, shape) {
  ...
  if (dimW > (aw + DvtMath.TOLERANCE) ||
    ...
    if (shape) {
      var dim = DvtDisplayableUtils.getDimensionsForced(context.getCtx(), shape);
...
DvtAfComponent.prototype._getUniqueId = function() {
  ...
  var id = this.getId();
  var rowIndex = this._context.getRowIndex();
  if (id === undefined) {
...
DvtAfComponent.prototype.RegisterImageCallback = function(imgSrc) {
  var owner = this._context.getImageCallbackObj();
  var func = this._context.getImageCallback();
  ...
  //Bug 16717528 - diagram nodes overlapped if images are not loaded
  func = this._context.getImageNotReadyCallback();
  if (owner && func) {
...
DvtAfComponent.prototype.associate = function(shape, logicalObject) {
  if (this._context.getEventManager())
    this._context.getEventManager().associate(shape, logicalObject ? logicalObject : this);
};
...
DvtAfComponent.prototype.CreateNewContainerForChildren = function() {
  ...
  if (dispObj.getNumChildren() > 0) {
    ...
    var newContainer = new DvtContainer(this._context.getCtx());
...
DvtAfComponent.prototype.HandleActiveElementChangeEvent = function(id) {
  if (this._context) {

      var callback = this._context.getContextCallback();
      var callbackObj = this._context.getContextCallbackObj();
      if (callback && callbackObj) {
        callback.call(callbackObj, new DvtActiveElementChangeEvent(DvtActiveElementChangeEvent.CHANGE, id));
      }
  }
};
...
DvtAfComponent.prototype.unregisterAsTabStop = function() {
  ...
  this._context.unregisterTabStop(this);
...
DvtAfComponent.prototype.showKeyboardFocusEffect = function() {
  ...
  this.HandleActiveElementChangeEvent(this.getId()); //accessibility feature
};
...
DvtAfComponent.prototype.CreateKeyboardFocusEffect = function() {
  return new DvtKeyboardFocusEffect(this._context.getCtx(), this.getDisplayObj(), this.getDisplayObj().getDimensions());
};
...
DvtAfComponent.prototype.HandleKeyboardEvent = function(event) {
  ...
  if (keyCode == DvtKeyboardEvent.ENTER) {
    var eventManager = this._context._eventManager;
    ...
    var mouseEvent = eventManager.GenerateMouseEventFromKeyboardEvent(
                     event, this._context.getCtx().getStage(), point.x, point.y);
 ...
 */
DvtAfComponent.prototype.getSize = function(context) {
  //stamp self, not recurses on its children
  var self = DvtAfComponent.superclass.stamp.call(this, context);
  var size = null;
 ...
 *
 * @param {DvtAfContext} context
 * @param {DvtAfComponent} afRoot The root of the afComponent tree.
 */
DvtAfComponentUtils._calcVContentSize = function(context, afRoot, arDim, isGoLink) {
  ...
  for (var i = 0; i < cnt; i++) {
    ...
    if (! child.renderMe || child.renderMe()) {
      ...
      if (child.getClipped() ||
        ...
        if (child.getClipped() == DvtAfComponent._REMOVE_LATER || context.getRmIfNotFit()) {
          toBeRm.push(child);
          continue;
        }
        else {
 ...
 * @param {DvtAfComponent} afRoot The root of the afComponent tree.
 */
DvtAfComponentUtils._calcHContentSize = function(context, afRoot, arDim) {
  var cnt = afRoot.getNumChildren();
  if (cnt == 0) {
    return new DvtRectangle(0, 0, 0, 0);
  }
 ...
 * @param {DvtAfComponent} afRoot The root of the afComponent tree.
 */
DvtAfComponentUtils.doVerticalLayout = function(context, afRoot) {
  if (! afRoot) {
    return;
  }

  var isGoLink = (afRoot instanceof DvtAfGoLink);
  var arDim = [];
  var afRootDim = DvtAfComponentUtils._calcVContentSize(context, afRoot, arDim, isGoLink);
  if (arDim.length == 0) {
    return;
  }
 ...
 */
DvtAfComponentUtils.doHorizontalLayout = function(context, afRoot) {
  if (! afRoot) {
    return;
  }
  var arDim = [];
  var afRootDim = DvtAfComponentUtils._calcHContentSize(context, afRoot, arDim);
  if (arDim.length == 0) {
    return;
  }
  ...
  if (DvtAgent.isRightToLeft(afRoot._context.getCtx())) {
 ...
 */
DvtAfTagHandler.prototype.getChildtagHandler = function() {
  return null;
};
...
DvtAfComponentFactory.DEFER_TRUNCATION = true;
 ...
 * @param {string} space the namespace of the tag
 * @param {string} tagName the name of the tag
 * @param {DvtTagHandler} tagHandler the handler for the tag
 */
DvtAfComponentFactory.registerComponent = function(space, tagName, tagHandler) {

  if (tagName && tagHandler) {
    var key = DvtAfComponentFactory._getKey(space, tagName);
    DvtAfComponentFactory._registeredTags[key] = tagHandler;
  }
};
...
DvtAfComponentFactory.parseXml = function(xmlNode) {
  var childList = DvtAfComponentFactory._parseChildren(xmlNode, null);
 ...
 * @param {DvtXmlNode} xml element
 * @param {DvtTagHandler} tagHandler Optional, the handler for the tag
 * @return {DvtAfComponent} the root of the DvtAfComponent tree
 */
DvtAfComponentFactory.parseElement = function(tagElement, tagHandler) {
  var tagComponent = null;

  // call the tagHandler to parse the element and create a AfTagComponent
  if (tagHandler && tagHandler.createComponent) {
    tagComponent = tagHandler.createComponent(tagElement);
  }

  // if tagHandler did not handled the tagElement, use the registered handler
  if (tagComponent == null) {
    //TODO space
    var space;
    var key = DvtAfComponentFactory._getKey(space, tagElement.getName());
    tagHandler = DvtAfComponentFactory._registeredTags[key];

    //TODO: no handler for this error
    if (! tagHandler) {
      throw 'No tag handler for ' + tagElement.getName();
    }
    tagComponent = tagHandler.createComponent(tagElement);
  }

  if (tagComponent != null) {
    var childTagHandler = null;

    // if there is a childTagHandler, pass it to the child tags
    if (tagHandler && tagHandler.getChildTagHandler) {
      childTagHandler = tagHandler.getChildTagHandler();
    }

    // get a list of child components
    var childList = DvtAfComponentFactory._parseChildren(tagElement, childTagHandler);
 ...
 */
DvtAfComponentFactory._parseChildren = function(xmlNode, tagHandler) {
  var childList = null;
  ...
  if (xmlNode) {
    ...
    for (var i = 0; i < childNodes.length; i++) {
      ...
      if (child && childName != null) {
        ...
        childComponent = DvtAfComponentFactory.parseElement(child, tagHandler);
        if (childComponent) {
...
/**
 * @private
 * return key to the registeredTags map
 */
DvtAfComponentFactory._getKey = function(space, tagName) {
  if (space === undefined)
    space = 'dvt';
  return space + ':' + tagName.toLowerCase();
};
...
/*---------------------------------------------------------------------*/
/**
 * @param {DvtAfContext} context may contain factory, available size, fontSize and ELcontext
 * @param {DvtXmlNode} xmlNode the XML node to parse
 ...
 */
DvtAfComponentFactory.parseXmlAndLayout = function(context, xmlNode, parent)
{
  // parse xmlNode into a DvtAfComponent tree
  var templateTree = DvtAfComponentFactory.parseXml(xmlNode);
  return DvtAfComponentFactory.parseAndLayout(context, templateTree, parent);
};
 ...
 * @return {DvtDisplayable} the root of displayable AfComponent tree
 */
DvtAfComponentFactory.parseAndLayout = function(context, templateTree, parent) {
...
//  if (context.getELContext())
    compTree = templateTree.stamp(context.getELContext());
//  else
//    compTree = templateTree;

  return DvtAfComponentFactory.doLayout(context, compTree, parent);
};
 ...
 * @return {DvtDisplayable} the root of displayable AfComponent tree
 */
DvtAfComponentFactory.doLayout = function(context, compTree, parent) {
  var dispTree = DvtAfComponentFactory._createDisplayObj(context, compTree, parent);

  // layout the displayable tree
  // Note: render code uses available size and font size if specified
  if (compTree) {
    DvtAfComponentFactory.beforeRender(context);

//     compTree.render(context);
    DvtAfComponentFactory._render(context, dispTree, compTree);

    DvtAfComponentFactory.afterRender(context);
  }
...
DvtAfComponentFactory.parseAndStamp = function(context, templateTree, parent) {
  // stamp out templateTree
  // Note: if xmlstring contains EL exps, ELcontext must provide data
  var compTree;
...
//Performance: make sure using the same context for parsing, stamping
//sizing, layout and rendering
DvtAfComponentFactory.beforeRender = function(context) {
  // getDimensions for all text components
  if (context) {
...
//Performance: make sure using the same context for parsing, stamping
//sizing, layout and rendering
DvtAfComponentFactory.afterRender = function(context) {
  // peform truncations on the text components that are overflow
  if (context && DvtAfComponentFactory.DEFER_TRUNCATION) {
    context.TruncateTexts();
  }
};
...
//Performance: make sure using the same context for parsing, stamping
//sizing, layout and rendering
DvtAfComponentFactory.layoutAndRender = function(context, dispTree) {
  if (dispTree) {
//     dispTree._compTree.render(context);

    // clear dirty flag
//    context._addDirty(-Infinity);

    DvtAfComponentFactory._render(context, dispTree, dispTree._compTree);
  }
};


// create displayable object tree and add to the parent
DvtAfComponentFactory._createDisplayObj = function(context, compTree, parent) {
  var dispTree;

  // create displayable object tree,
  // Note: context must contain factory
  if (compTree) {
    dispTree = compTree.createDisplayObj(context);
  }
...
DvtAfComponentFactory._render = function(context, dispTree, compTree) {
  if (compTree) {
    compTree.render(context);
    compTree.postRender(context);
    ...
    if (context.getRmIfNotFit()) {
      ...
      if ((parent && parent.removeChild) &&
          ((dim.w === 0 && dim.h === 0) ||
           dim.w > (context.getAvailableWidth() + DvtMath.TOLERANCE) ||
           dim.h > (context.getAvailableHeight() + DvtMath.TOLERANCE) ||
           compTree.getClipped())) {
...
//    if (context._isDirty()) {
      ...
      // clear dirty flag
//      context._addDirty(-Infinity);
//      compTree._render(context, dispTree, compTree);
//    }
...
DvtAfComponentFactory.getSize = function(context, templateTree, parent) {

  context.setParentNode(parent);

  return templateTree.getSize(context);
};
...
/*---------------------------------------------------------------------*/
DvtAfButtonLink._createContentShape = function(icontext, afComp, mouseState, textStyle) {
  ...
  if (! isLink) {
    group = new DvtRect(icontext, 0, 0, 0, 0, id);
  }
  else if (iconSrc && text) {
    group = new DvtContainer(icontext, id);
  }

  var iconShape;
  var textShape;
  if (iconSrc) {
    iconShape = new DvtImage(icontext, iconSrc, 0, 0, 1, 1, id + '_img');
  ...
  if (text) {
    ...
    //Note: since we did our own layout, make sure it's  align start
    //also, remove the default cursor setting if this component is a link
    textShape.alignLeft();
    if (isLink) {
...
DvtAfButtonLink._doLayout = function(afComp, mouseState, textStyle, arrowWidth) {
  ...
  var shape = afComp._shapeLst[mouseState];
  ...
  case 'trailing':
    ...
    if (DvtAgent.isRightToLeft(afComp._context.getCtx())) {
      if (iconPos == 'leading')
        iconPos = 'trailing';
      else
        iconPos = 'leading';
  ...
  if (textShape) {
    ...
    textShape.setMaxWidth(txtMaxWidth);

    textDim = textShape.getCachedDims();
  ...
  case 'trailing':
    ...
    textDim.x = xx + arrowW;
    iconDim.x = txtw + hpadding + xx + arrowW;
  ...
  case 'leading':
    ...
    textDim.x = iconDim.w + hpadding + xx + arrowW;
    iconDim.x = xx + arrowW;
  ...
  if (textShape) {
    ...
    if (isTruncated) {
      // cached the shape for text truncation later
      if (afComp.deferTruncation()) {
        afComp._context.AddObjShapePair(textShape.getId(), textShape);
      }
    }
    else {
      // add clip path if needed
      afComp._addClipPathIfNeeded(afComp._context, xx, yy,
...
DvtAfButtonLink.prototype.AddLink = function() {
  //add "a" element around the text
  var target = this.getTargetFrame();
  ...
  if ((target || dest)) {
    ...
    var linkObj = {};
    linkObj[DvtAfButtonLink.ATTR_DESTINATION] = dest;
    linkObj[DvtAfButtonLink.ATTR_TARGET_FRAME] = target;
    linkObj[DvtAfButtonLink.ATTR_FOCUSABLE] = 'false';

    //Bug 15865017 - GET RID OF DVTGOLINK AND IMPLS
    shape.setHyperlink(linkObj);
  }
};
...
/*   Create Content Shape                                              */
/*---------------------------------------------------------------------*/
DvtAfButtonLink.prototype.createContentShape = function(icontext) {
...
DvtAfButtonLink.prototype.CreateButton = function(button) {
  ...
  if (! button) {
    return new DvtButton(this._context.getCtx(), null, null, null, null,
                         this._getUniqueId());
  }
...
DvtAfButtonLink.prototype.CreateButtonState = function(buttonState) {
  ...
  var bgRect = DvtAfButtonLink._createContentShape(this._context.getCtx(),
                                                   this, buttonState,
                                                   eStyle);
...
DvtAfButtonLink.prototype.RenderUpState = function() {
  var ss = this.GetEnabledStyle();
  return this.RenderButtonState(ss, DvtButton.STATE_ENABLED);
};
...
DvtAfButtonLink.prototype.RenderDownState = function() {
  var ss = this.GetActiveStyle();

  return ss ? this.RenderButtonState(ss, DvtButton.STATE_DOWN) : null;
};
...
DvtAfButtonLink.prototype.RenderOverState = function() {
  var ss = this.GetHoverStyle();
  return ss ? this.RenderButtonState(ss, DvtButton.STATE_OVER) : null;
};
...
DvtAfButtonLink.prototype.RenderDisabledState = function() {
  var ss = this.GetDisabledStyle();
  return ss ? this.RenderButtonState(ss, DvtButton.STATE_DISABLED) : null;
};
 ...
 * @return display obj to use for button state
 */
DvtAfButtonLink.prototype.RenderButtonState = function(style, buttonState) {

  if (style) {
    style.clone().merge(this.getCSSStyle());
  }
  else {
    style = this.getCSSStyle();
  }
...
DvtAfButtonLink.prototype.registerAsTabStop = function() {
  this._context.registerTabStop(this);
};
/**
 * @constructor
 * DvtAfButton
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfButton.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfButton));
...
DvtAfButton.prototype.SetDefaultStyles = function(styleList) {
  ...
  // border radius
  var br = styleList[0].getStyle(DvtCSSStyle.BORDER_RADIUS);
  if (styleList.length > 7) {
    styleList[7].mergeUnder(styleList[5]);
    if (! styleList[7].getStyle(DvtCSSStyle.BORDER_RADIUS)) {
      styleList[7].setStyle(DvtCSSStyle.BORDER_RADIUS, br);
    }
  }
  if (styleList.length > 8) {
    styleList[8].mergeUnder(styleList[5]);
    if (! styleList[8].getStyle(DvtCSSStyle.BORDER_RADIUS)) {
      styleList[8].setStyle(DvtCSSStyle.BORDER_RADIUS, br);
    }
  }
...
DvtAfButton.prototype.RenderBackgroundImage = function(bgShape, style, bgImg) {
  if (bgImg && 'none' !== bgImg && (! (bgImg instanceof DvtCSSGradient))) {
    ...
    if (! bgRepeat || bgRepeat != 'no-repeat') {
      ...
      var dim = bgShape.getDimensions(this._context.getParentNode());
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfCommandButton.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfCommandButton));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfGoButton.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfGoButton));
 ...
 * Handle "enter" event
 */
DvtAfGoButton.prototype.HandleKeyboardEvent = function(event) {
  var keyCode = event.keyCode;
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfLink.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfLink));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfCommandLink.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfCommandLink));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfCommandImageLink.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfCommandImageLink));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfGoLink.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfGoLink));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfGoLink.prototype.createDisplayObj = function(context) {
  var dispObj = DvtAfGoLink.superclass.createDisplayObj.call(this, context);

  this.addChildDisplayObjs(context, this._shape);

  return dispObj;
};


DvtAfGoLink.prototype.renderSelfBeforeChildren = function() {
  DvtAfGoLink.superclass.renderSelfBeforeChildren.call(this);
 ...
 * @override
 */
DvtAfGoLink.prototype.doLayout = function() {
  if (this.getNumChildren() > 0)
    DvtAfComponentUtils.doVerticalLayout(this._context, this);
};


DvtAfGoLink.prototype.renderChild = function(child) {
  if (child._render) {
    ...
    var context = this._context;
    DvtAfComponent._renderComp(context, child, this._acw, this._ach);
  }
 ...
 * @private
 */
DvtAfGoLink.prototype.getHAlign = function() {
 ...
 * Handle "enter" event
 */
DvtAfGoLink.prototype.HandleKeyboardEvent = function(event) {
  var keyCode = event.keyCode;
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfCommandMenuItem.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfCommandMenuItem));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfMenu.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfMenu));
...
DvtAfMenu.prototype.CreateButtonState = function(mouseState) {
  ...
  if (group && arrowStyle) {
    ...
    if (arrowSrc) {
      ...
      this._arrowLst[mouseState] = new DvtImage(this._context.getCtx(), arrowSrc, 0, 0,
...
DvtAfMenu.prototype._RenderArrowIcon = function(mouseState, textIconDim) {
  ...
  if (ashape) {
/*
    var size = DvtImageLoader.loadImage(this._context.getCtx(),
                                        ashape.getSrc(),
                                        this.createCallback(this.onArrowIconLoaded));
*/
...
/*
DvtAfMenu.prototype.onArrowIconLoaded = function(size) {
  // set size on the arrow shape
  if (size != null && size.width && size.height) {
    var ashape = this._arrowLst[mouseState];
    ashape.setWidth(size.width);
    ashape.setHeight(size.height);
  }
}
*/
...
DvtAfMenu.prototype.displayDropdown = function() {
  ...
  var dropdown = new DvtRect(this._context.getCtx(), 0, 0, menuDim.w, 1, id + '_dd');
  ...
  this._context.setContextCallback(this.getContextCallbackObj(),
                                   this.getContextCallback());
  this.populateDropdown(this._context, dropdown);
...
//      point = hoverWindow.stageToLocal(point);
  ...
  var stage = this._context.getCtx().getStage();
  if (stage) {
...
DvtAfMenu.prototype._logEvent = function(event) {
//  logEvent(event);
};
...
DvtAfMenu.prototype._getMenuDropdownContainer = function(create) {
  var container = this._context.getMenuDropdownContainer();
  if (create && ! container) {
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfImage.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfImage));
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfImage.prototype.createContentShape = function(icontext) {
  //Bug 13384179 - af:image border-color and border-width don't work
  return new DvtImage(icontext, this.getSrc(), 0, 0, 1, 1, this._getUniqueId());
};
...
DvtAfImage.prototype.adjustClientArea = function(cssStyle) {
  if (this.getSrc()) {
    ...
    var image;
    image = DvtImageLoader.loadImage(this._context.getCtx(),
                                     this.getSrc(),
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfMarker.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfMarker));
...
/**
 * Create a DvtMarker for this component
 * @param {DvtAfContext} context may contain available size, fontSize and ELcontext
 * @return {DvtMarker} the display object for this component
 */
DvtAfMarker.prototype.createDisplayObj = function(context) {
  ...
  // create marker
  var marker = new DvtMarker(icontext, type, null, x, y, ww, hh, id + '_m', scaleX, scaleY);
  ...
  if (value && bvis) {
    var labelId = id + '_lb';
    var label = new DvtText(icontext, value, 0, 0, labelId);

    // add to a list of obj that need to getDimensions
    if (DvtAfComponentFactory.DEFER_TRUNCATION) {
      context.AddObjShapePair(labelId, label);
    }
    var cssStyle;
    var lstyle = this.getLabelStyle();
    if (DvtAfComponentFactory.CACHE_STYLES) {
      cssStyle = context.GetCSSStyle(this.getTagName(), '', lstyle, true);
    }
    else {
      cssStyle = lstyle ? new DvtCSSStyle(lstyle) : DvtAfStyleUtils.getDefaultTextStyle();
    }
    ...
    var group = new DvtContainer(icontext, id);
...
DvtAfMarker.prototype.render = function(context) {
  //Bug 13477511 - RENDERED FLAG ISN'T WORKING CORRECTLY FOR MARKERS
  if (! this.renderMe())
    return;
  ...
  if (! label) {
    //extends DvtAfComponent
    this._setBounds(context, mkx, mky, msw, msh);
    return;
  }

  //Bug 13408941 - diagrammer js code stops executing when markersize is zero
  if (msw === 0 && msh === 0) {
    this._setBounds(context, mkx, mky, 0, 0);
    return;
  }
  ...
  //extends DvtAfComponent
  this._setBounds(context, lbtx, lbty, gww, ghh);
...
DvtAfMarker.prototype._render = function(availWidth, availHeight) {
  ...
  //so we have a list of dimensions for all text objects
  var context = this._context;
  DvtAfComponent._renderComp(context, this, availWidth, availHeight);
};
...
DvtAfMarker.prototype._setBounds = function(context, xx, yy, ww, hh) {
  this._dim = new DvtRectangle(xx, yy, ww, hh);

  // clipped?
  this._addClipPathIfNeeded(context, 0, 0,
                            context.getAvailableWidth(), this._dim.w,
                            context.getAvailableHeight(), this._dim.h);
};
...
DvtAfMarker.prototype.CreateKeyboardFocusEffect = function() {
  var matrix = this.getDisplayObj().getMatrix();
  return new DvtKeyboardFocusEffect(this._context.getCtx(), this.getDisplayObj(), this.getDisplayObj().getDimensions(), matrix);
};
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfOutputText.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfOutputText));
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfOutputText.prototype.createContentShape = function(icontext) {
...
DvtAfOutputText.prototype.renderSelfBeforeChildren = function() {
  ...
  if (this.getValue()) {
    ...
    //BUG FIX 13620233: set max height on DvtTextArea so we know when to
    //stop wrapping
    if (shape.setMaxHeight) {
      shape.setMaxHeight(this._ch);
    }
    ...
    var isTruncated = shape.isTruncated();
    dim = shape.getCachedDims();

    // Do not render if not added to the DOM
    if (dim) {
      // add clip path if needed
      if (! isTruncated) {
        this._addClipPathIfNeeded(this._context, this.getCX(), this.getCY(),
      ...
      if (isTruncated) {
        // cached the shape for text truncation later
        if (this.deferTruncation()) {
          this._context.AddObjShapePair(shape.getId(), shape);
        }
        this.associate(shape, this);
      }
    }
...
DvtAfOutputText.prototype.deferTruncation = function() {
  return DvtAfComponentFactory.DEFER_TRUNCATION && this.isNoWrap();
};
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfOutputFormatted.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfOutputFormatted));
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfOutputFormatted.prototype.createContentShape = function(icontext) {
  var context = this._context;
  var id = this._getUniqueId();
  textShape = new DvtTextFormatted(icontext, 0, 0, id);
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfPanelGroupLayout.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfPanelGroupLayout));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfPanelGroupLayout.prototype.createDisplayObj = function(context) {
  var dispObj = DvtAfPanelGroupLayout.superclass.createDisplayObj.call(this, context);

  this.addChildDisplayObjs(context, this._contentPane);
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfPanelGroupLayout.prototype.createContentShape = function(icontext) {
  this._contentPane = new DvtRect(icontext, 0, 0, 0, 0, this._getUniqueId());
...
/*   Create background Shape                                           */
/*---------------------------------------------------------------------*/
DvtAfPanelGroupLayout.prototype.createBackgroundShape = function(icontext, shape) {
  return shape;
};
...
DvtAfPanelGroupLayout.prototype.doLayout = function() {
  ...
  if (this.getLayout() == DvtAfPanelGroupLayout.LAYOUT_VERTICAL) {
    DvtAfComponentUtils.doVerticalLayout(context, this);
  }
  else {
    DvtAfComponentUtils.doHorizontalLayout(context, this);
  }
};
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfPanelFormLayout.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfPanelFormLayout));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfPanelFormLayout.prototype.createDisplayObj = function(context) {
  var dispObj = DvtAfPanelFormLayout.superclass.createDisplayObj.call(this, context);
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfPanelFormLayout.prototype.createContentShape = function(icontext) {
  this._contentPane = new DvtRect(icontext, 0, 0, 0, 0, this._getUniqueId() + '_cp');
...
DvtAfPanelFormLayout.prototype.renderChild = function(child) {
  ...
  if (child._render) {
    ...
    //Performance: using the same context during rendering
    //so we have a list of dimensions for all text objects
    DvtAfComponent._renderComp(this._context, child, ww, this._ach);
  }
  ...
  for (var i = 0; i < cnt; i++) {
    ...
    if (child instanceof DvtAfPanelLabelAndMessage) {
      ...
      if (labelStyle) {
        ...
        textAlignEnd = labelStyle.isTextAlignEnd(context.getCtx());

        //for R2L locale, pad right side of label
        //for L2R locale, pad left side of label
        paddingLR = DvtAgent.isRightToLeft(this._context.getCtx()) ? labelStyle.getPadding(DvtCSSStyle.PADDING_RIGHT) :
      ...
      childx = currX + paddingLR + (DvtAgent.isRightToLeft(this._context.getCtx()) ? this._maxLabelWidth : 0);
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfPanelLabelAndMessage.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfPanelLabelAndMessage));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfPanelLabelAndMessage.prototype.createDisplayObj = function(context) {
  var dispObj = DvtAfPanelLabelAndMessage.superclass.createDisplayObj.call(this, context);
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfPanelLabelAndMessage.prototype.createContentShape = function(icontext) {
  this._contentPane = new DvtRect(icontext, 0, 0, 0, 0, this._getUniqueId() + '_cp');
  this._contentPane.setFill(null);
...
DvtAfPanelLabelAndMessage.prototype.renderSelfBeforeChildren = function() {
  ...
  if (labelField) {
    labelField.setMaxWidth(this._acw);
...
//   var maxChildHeight = this.getCalcHeight(afRootDim.h);
  ...
  var labelStyle = this._getLabelStyle();
  var labelWidth = labelStyle.isTextAlignEnd(this._context.getCtx()) ? lbDim.w : this._maxLabelWidth;
  ...
  var dim = DvtDisplayableUtils.getDimensionsForced(this._context.getCtx(), this.getContentShape());
  this._cw = this.getCalcWidth(dim.w);
  this._ch = this.getCalcHeight(dim.h);
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfSpacer.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfSpacer));
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfSpacer.prototype.createContentShape = function(icontext) {
  this._contentPane = new DvtRect(icontext, 0, 0, 1, 1, this.getId());
  this._contentPane.setFill(null);
  return this._contentPane;
};
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfSeparator.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfSeparator));
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfSeparator.prototype.createContentShape = function(icontext) {
  // create a container for separator
  var id = this._getUniqueId();
  var group = new DvtContainer(icontext, id);

  this._line1 = new DvtLine(icontext, 0, 0, 1, 0, id + '_1');
  this._line2 = new DvtLine(icontext, 0, 1, 1, 1, id + '_2');
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfShowDetailItem.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfShowDetailItem));
...
/*   Create Content Shape                                              */
/*---------------------------------------------------------------------*/
DvtAfShowDetailItem.prototype.createContentShape = function(icontext) {
  //Initialize instance variables
  //Note: this._loaded is initialized to false if panelCard.contentDelivery = lazy
  this._loaded = true;

  this._contentPane = new DvtContainer(icontext, this._getUniqueId());
...
DvtAfShowDetailItem.prototype.addChildDisplayObjs = function(context, parentDO) {
  if (! this.IsLoaded())
    return;

  DvtAfShowDetailItem.superclass.addChildDisplayObjs.call(this, context, parentDO);
}
*/
 ...
 */
DvtPanelCardButtonLAFUtils.createLeftArrowState = function(context, state, buttonStyles) {

  var s = DvtPanelCardButtonLAFUtils.buttonShape(context, state, 'la' + state, buttonStyles);
  DvtPanelCardButtonLAFUtils.make_leftarrow_Icon(context, s, state, buttonStyles);
 ...
 */
DvtPanelCardButtonLAFUtils.createRightArrowState = function(context, state, buttonStyles) {

  var s = DvtPanelCardButtonLAFUtils.buttonShape(context, state, 'ra' + state, buttonStyles);
  DvtPanelCardButtonLAFUtils.make_rightarrow_Icon(context, s, state, buttonStyles);
...
DvtPanelCardButtonLAFUtils.buttonShape = function(context, state, id, buttonStyles) {
...
DvtPanelCardButtonLAFUtils.make_leftarrow_Icon = function(context, parent, st, buttonStyles) {
  var ww = 6;
  var hh = 9;
...
DvtPanelCardButtonLAFUtils.make_rightarrow_Icon = function(context, parent, st, buttonStyles) {
  var ww = 6;
  var hh = 9;
  ...
  var shape = new DvtPath(context, cmds, 'rightArrow');
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfPanelCard.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfPanelCard));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfPanelCard.prototype.createDisplayObj = function(context) {

  this._setNode(context.getParentNode());
    var dispObj = DvtAfPanelCard.superclass.createDisplayObj.call(this, context);
...
DvtAfPanelCard.prototype.addChildDisplayObjs = function(context, parentDO) {
  ...
  if (sdi && (! sdi.renderMe || sdi.renderMe())) {
    childDO = sdi.createDisplayObj(context);
...
/*   Create Content Shape                                                  */
/*-------------------------------------------------------------------------*/
DvtAfPanelCard.prototype.createContentShape = function(icontext) {
  var style = this.getCSSStyle();
  ...
  switch (this.getNodeShape()) {
    ...
    case DvtAfPanelCard.SHAPE_ELLIPSE:
      this._contentPane = new DvtPath(icontext, '', this._getUniqueId());
      break;
...
/*   Create background Shape                                           */
/*---------------------------------------------------------------------*/
DvtAfPanelCard.prototype.createBackgroundShape = function(icontext, shape) {
  return shape;
};
...
DvtAfPanelCard.prototype.displayDetailItem = function(index) {
  ...
  var bForward = ((index > disIndex) && ! DvtAgent.isRightToLeft(this._context.getCtx())) ||
    ((index < disIndex) && DvtAgent.isRightToLeft(this._context.getCtx()));
...
DvtAfPanelCard.prototype.renderSelfBeforeChildren = function() {
  ...
  //there's only one showDetailItem
  if (this.needsNavUi()) {
...
DvtAfPanelCard.prototype.renderSelfAfterChildren = function() {
  ...
  var padding = DvtAfPanelCard.BUTTON_PADDING;
  ...
  if (prevButton) {
    ...
    prevButton.setTranslateX(this.getCX() + padding);
    prevButton.setTranslateY(this.getCY() + this._ch - prevDim.h - padding);
  }
  ...
  if (nextButton) {
    ...
    nextButton.setTranslateX(this.getCX() + this._cw - nextDim.w - padding);
    nextButton.setTranslateY(this.getCY() + this._ch - nextDim.h - padding);
  }
  ...
  if (labels.length <= DvtAfPanelCard.MAX_TRAIN_ITEM_COUNT) {
    ...
    //there's only one showDetailItem
    if (this.needsNavUi()) {
      train = new DvtTrain(this._context.getCtx(), this._context.getEventManager(),
    ...
    var prevRightX = 0;
    if (prevButton) {
      prevRightX = prevButton.getTranslateX() + prevDim.w;
    }

    if (train) {
      rootDO.addChild(train);
      var trainDim = train.getDimensions();
      this._train = train;
      ...
      if (trainDim.w <= Math.round(nextButton.getTranslateX() - prevRightX)) {
        train.setTranslateX(prevRightX +
                            ((nextButton.getTranslateX() - prevRightX) - trainDim.w) / 2);
...
DvtAfPanelCard.prototype.doLayout = function() {
  ...
  if (this._titleBar) {
    ...
    var xx = this.getCX();
    if (((align == 'end' || align == 'right') && ! DvtAgent.isRightToLeft(this._context.getCtx())) ||
        ((align == 'start' || align == 'left') && DvtAgent.isRightToLeft(this._context.getCtx()))) {
      xx += this._cw - tbDim.w;
    }
 ...
 *
 * @return true if the navigation UI is necessary, false if not
 */
DvtAfPanelCard.prototype.needsNavUi = function() {
  //if there is only one showDetailItem, look for the Sherman skinning
...
DvtAfPanelCard.prototype.renderDetailItem = function(bForward) {
  ...
  this._context.setTabStopsArray(this._getNode().getTabStopsArray());
...
//    this._getGroupDO().addChildAt(titleBar, 1);
  ...
  if (this._saveContext) {
    this._context.TruncateTexts();
    this._context = this._saveContext;
    this._saveContext = undefined;
  }
  ...
  this.TransitionDisplayItems(effect, bForward);
};

DvtAfPanelCard.prototype.CreatePrevButton = function(context) {
  this._prevButton = new DvtNavButton(this,
                                      this._context.getCtx(),
  ...
  this._prevButton.addEvtListener(DvtTouchEvent.TOUCHSTART, this.HandlePrevButtonClick, false, this);

  if (!DvtAgent.isTouchDevice()) {
    this._prevButton.addEvtListener(DvtMouseEvent.CLICK, this.HandlePrevButtonClick, false, this);
    this._prevButton.addEvtListener(DvtMouseEvent.MOUSEOVER, this.HandlePrevButtonRollOver, false, this);
    this._prevButton.addEvtListener(DvtMouseEvent.MOUSEOUT, this.HandlePrevButtonRollOut, false, this);
    this._prevButton.addEvtListener(DvtMouseEvent.MOUSEDOWN, this.ConsumeEvent, false, this);
  }
};
...
DvtAfPanelCard.prototype.CreateNextButton = function() {
  ...
  this._nextButton.addEvtListener(DvtTouchEvent.TOUCHSTART, this.HandleNextButtonClick, false, this);

  if (!DvtAgent.isTouchDevice()) {
    this._nextButton.addEvtListener(DvtMouseEvent.CLICK, this.HandleNextButtonClick, false, this);
    this._nextButton.addEvtListener(DvtMouseEvent.MOUSEOVER, this.HandleNextButtonRollOver, false, this);
    this._nextButton.addEvtListener(DvtMouseEvent.MOUSEOUT, this.HandleNextButtonRollOut, false, this);
    this._nextButton.addEvtListener(DvtMouseEvent.MOUSEDOWN, this.ConsumeEvent, false, this);
  }

  this._getGroupDO().addChild(this._nextButton);
};
...
/**
 * Remove listeners from the prev button.
 */
DvtAfPanelCard.prototype.removePrevButtonListeners = function() {
  this._prevButton.removeEvtListener(DvtMouseEvent.CLICK, this.HandlePrevButtonClick, false, this);
  this._prevButton.removeEvtListener(DvtMouseEvent.MOUSEOVER, this.HandlePrevButtonRollOver, false, this);
  this._prevButton.removeEvtListener(DvtMouseEvent.MOUSEOUT, this.HandlePrevButtonRollOut, false, this);
  this._prevButton.removeEvtListener(DvtMouseEvent.MOUSEDOWN, this.ConsumeEvent, false, this);
};

/**
 * Remove listeners from the next button.
 */
DvtAfPanelCard.prototype.removeNextButtonListeners = function() {
  this._nextButton.removeEvtListener(DvtMouseEvent.CLICK, this.HandleNextButtonClick, false, this);
  this._nextButton.removeEvtListener(DvtMouseEvent.MOUSEOVER, this.HandleNextButtonRollOver, false, this);
  this._nextButton.removeEvtListener(DvtMouseEvent.MOUSEOUT, this.HandleNextButtonRollOut, false, this);
  this._nextButton.removeEvtListener(DvtMouseEvent.MOUSEDOWN, this.ConsumeEvent, false, this);
};
...
DvtAfPanelCard.prototype.destroy = function() {
  ...
  if (this._prevButton) {
    this.removePrevButtonListeners();
  }
  if (this._nextButton) {
...
DvtAfPanelCard.prototype.CreateButtonIconState = function(iconSrc, buttonState, bNext) {
  var context = this._context.getCtx();
  var iconShape = new DvtImage(context, iconSrc, 0, 0, 1, 1);
/*
  var onLoadFunc = null;
  ...
  if (onLoadFunc) {
*/
    var isize = iconShape.loadImage();
    if (isize) {
      iconShape.setWidth(isize.w);
      iconShape.setHeight(isize.h);
    }
    else {
      this.RegisterImageCallback(iconSrc);
    }
...
DvtAfPanelCard.prototype.CreateButtonState = function(bNext, state) {
  ...
  var context = this._context.getCtx();
  if (bNext)
    s = DvtPanelCardButtonLAFUtils.createRightArrowState(context, state, this.getCardNavButtonStyles());
  else
    s = DvtPanelCardButtonLAFUtils.createLeftArrowState(context, state, this.getCardNavButtonStyles());
  return s;
};
...
DvtAfPanelCard.prototype._renderIcon = function(iconUrl) {
  var context = this._context.getCtx();
  DvtImageLoader.loadImage(context, iconUrl,
...
DvtAfPanelCard.prototype.CreateTitleBar = function(sdi) {
  ...
  var titleBar = new DvtContainer(context, id + '_tBar');

  var iconUrl = sdi.getIcon();
  if (iconUrl && iconUrl.length > 0) {
    this._icon = new DvtImage(context, iconUrl, 0, 0, 1, 1, id + '_icon');
...
DvtAfPanelCard.prototype.positionIconAndTextField = function() {
  // if icon specified, load image and set image (x, y)
  var isLocaleR2L = DvtAgent.isRightToLeft(this._context.getCtx());
  ...
  if (this._text) {
    ...
    textw = textDim.w;
    texth = textDim.h;
    this._text.setTranslateX(currX + 2);
  }
  ...
  if (isLocaleR2L) {
    ...
    if (this._text)
      this._text.setTranslateX(origWidth - this._text.getTranslateX() - textw);
  }
...
DvtAfPanelCard.prototype.removeOldDisplayItem = function(forced) {
  ...
  if (this._oldTitleBar && (forced || this._oldTitleBar !== this._titleBar)) {
    ...
    rootDO.removeChild(this._oldTitleBar);
...
DvtAfPanelCard.prototype.StartPopupTimer = function() {
  this._popupTimer = new DvtTimer(this._context.getCtx(),
                                  DvtAfPanelCard.POPUP_TIMER_INTERVAL,
                                  this.HandlePopupTimer, this, 1);
  this._popupTimer.start();
};

DvtAfPanelCard.prototype.StopPopupTimer = function() {
  if (this._popupTimer) {
...
DvtAfPanelCard.prototype.HandlePopupTimer = function(event) {
  //TODO: Create a popup menu when the mouse hovers over a navigation button.
  //showpopup here
/*
  if (this._bNextPopup)
    this._popupMenu = CreatePopup(this._nextButton);
  else
    this._popupMenu = CreatePopup(this._prevButton);
*/
};

DvtAfPanelCard.prototype.HandlePrevButtonRollOver = function(event) {
  this._bNextPopup = false;
  this.StartPopupTimer();
};

DvtAfPanelCard.prototype.HandlePrevButtonRollOut = function(event) {
  this.StopPopupTimer();
};

DvtAfPanelCard.prototype.HandleNextButtonRollOver = function(event) {
  this._bNextPopup = true;
  this.StartPopupTimer();
};

DvtAfPanelCard.prototype.HandleNextButtonRollOut = function(event) {
  this.StopPopupTimer();
};
...
DvtAfPanelCard.prototype.getNavButtonTooltip = function(button) {
  ...
  var bidi = DvtAgent.isRightToLeft(this._context.getCtx());
  var index = this.getDisplayedDetailItemIndex();
...
DvtAfPanelCard.prototype._render = function(availWidth, availHeight) {
  ...
  // add event listeners
  var cbObj = this._context.getPanelCardCallbackObj();
  var cbFunc = this._context.getPanelCardCallback();
...
DvtAfPanelCard.prototype._loadDetailItem = function(sdi, reload) {
  ...
  if (reload || (! sdi.getDisplayObj())) {
    ...
    if (diagram) {
      ...
      this._saveContext = this._context;
      this._context = afContext;
...
DvtAfPanelCard.prototype._insertChild = function(oldsdi, newsdi) {
  ...
  //add newsdi disp object
  this.getDisplayObj().addChildAt(newsdi.getDisplayObj(), index);
 ...
 *        false if moving to the previous detail item
 */
DvtAfPanelCard.prototype.TransitionDisplayItems = function(effect, bForward) {
...
DvtAfPanelCard.prototype.TransitionSlideHorizontal = function(bForward) {
  ...
  if (oldObjs.length || newObjs.length) {
    animator = new DvtCombinedAnimMoveBy(this._context.getCtx(), oldObjs, newObjs,
...
DvtAfPanelCard.prototype.createCombinedFlip = function(oldDispObj, newDispObj, bForward) {
  ...
  var playable = new DvtCombinedAnimFlip(this._context.getCtx(),
...
DvtAfPanelCard.prototype.createCombinedCube = function(oldDispObj, newDispObj, bForward) {
  ...
  var playable = new DvtCombinedAnimCube(this._context.getCtx(),
                                         oldDispObj, newDispObj, axis, rotationDir,
...
var DvtNavButton = function(panelCard, context, upState, overState, downState, id) {
  this.Init(panelCard, context, upState, overState, downState, id);
};

DvtObj.createSubclass(DvtNavButton, DvtButton, 'DvtNavButton');

DvtNavButton.prototype.Init = function(panelCard, context, upState, overState, downState, id) {
  DvtNavButton.superclass.Init.call(this, context, upState, overState, downState, id);
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfShowPopupBehavior.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfShowPopupBehavior));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfShowPopupBehavior.prototype.createDisplayObj = function(context) {
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfClientBehavior.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfClientBehavior));
...
/*   CreateDisplayObject                                                   */
/*-------------------------------------------------------------------------*/
DvtAfClientBehavior.prototype.createDisplayObj = function(context) {
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfGroup.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfGroup));
...
DvtAfComponentFactory.registerComponent('dvt', DvtAfPointLocation.TAG_NAME,
                                        new DvtAfTagHandler(DvtAfPointLocation));
