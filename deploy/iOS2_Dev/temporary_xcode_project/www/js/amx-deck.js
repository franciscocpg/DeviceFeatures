...
(function()
{
  ...
  deck.prototype.updateChildren = function(amxNode, attributeChanges)
  {
    // Handle if and only if "displayedChild" changed:
    if (attributeChanges.hasChanged("displayedChild"))
    {
  ...
  {
    // TODO - We need an improved mechanism for detecting dimensionsFromParent...
    if (adf.mf.internal.amx.getCSSClassNameIndex(rootElement.className, "amx-stretched") == -1)
    {
      ...
      {
        adf.mf.internal.amx.addCSSClassName(rootElement, "amx-stretched");
      }
    }
  };

  deck.prototype.refresh = function(amxNode, attributeChanges)
  {
    try
    {
      var rootElement = document.getElementById(amxNode.getId());
      var dimensionsFromParent = adf.mf.internal.amx.getCSSClassNameIndex(rootElement.className, "amx-stretched") != -1;
      var items = rootElement.firstChild;

      if (adf.mf.internal.amx.getCSSClassNameIndex(rootElement.className, "changingDisplayedChild") != -1)
      {
      ...
      adf.mf.internal.amx.addCSSClassName(rootElement, "changingDisplayedChild");
      ...
      else // no animation
      {
        ...
        adf.mf.internal.amx.removeCSSClassName(rootElement, "changingDisplayedChild");
      }
    }
    catch (problem)
    {
